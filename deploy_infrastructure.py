...
def deploy_ecs():
    ...
    uid = generate_id()
    repo_name = f"node-ecs-repo-{uid}"
    cluster_name = f"NodeAppCluster-{uid}"
    task_family = f"node-task-{uid}"
    service_name = f"node-service-{uid}"
    sg_name = f"ecs-node-sg-{uid}"
    role_name = f"ecsTaskExecutionRoleNode-{uid}"

    print("Starting Containerized Node.js App Deployment...")
    ...
    run_command("docker build -t node-ecs-app .")
    
    print("[INFO] Tagging and Pushing Docker image to ECR...")
    run_command(f"docker tag node-ecs-app:latest {repository_uri}:latest")
    ...
    try:
        ...
        if 'EntityAlreadyExists' in str(e):
            ...
            print(f"[INFO] IAM Role creation skipped: {e}")
    ...
    sg = ec2.create_security_group(GroupName=sg_name, Description='Allow Port 3000 for Node container', VpcId=vpc_id)
    sg_id = sg['GroupId']
    ec2.authorize_security_group_ingress(GroupId=sg_id, IpPermissions=[{'IpProtocol': 'tcp', 'FromPort': 3000, 'ToPort': 3000, 'IpRanges': [{'CidrIp': '0.0.0.0/0'}]}])
    ...
    print(f"\nAccess your Containerized Node.js App here: http://{public_ip}:3000")
    print("============================================================")
